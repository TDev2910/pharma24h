<template>
  <div class="page-container">
    <h1 class="page-title">Báo cáo trả hàng</h1>
            ...
            <table>
              ...
              <tbody>
                <tr v-for="(product, index) in filteredProducts" :key="`${product.type}-${product.id}`">
                  ...
                  <td><strong>{{ product.returned.toLocaleString() }}</strong></td>
...
export default {
  ...
  props: {
    topProducts: {
      ...
      default: () => [
        { id: 1, name: 'Panadol', type: 'medicine', returned: 600, price: 10000, stocks: 100, total_cost: 6000000 },
        { id: 2, name: 'Khẩu trang 4D', type: 'goods', returned: 1000, price: 5000, stocks: 5000, total_cost: 5000000 },
        { id: 3, name: 'Efferalgan', type: 'medicine', returned: 400, price: 15000, stocks: 200, total_cost: 6000000 },
        { id: 4, name: 'Bông băng', type: 'goods', returned: 500, price: 2000, stocks: 1000, total_cost: 1000000 },
    ...
    return {
      ...
      chartOptions: {
        ...
        plugins: {
          ...
          title: {
            display: true,
            text: 'Tổng số lượng trả theo loại',
            padding: { bottom: 20 },
            font: { weight: 'normal' }
          }
        },
  ...
  computed: {
    ...
    chartData() {
      ...
      // 1. Tính tổng số lượng trả của THUỐC
      const totalMedicine = filtered
        .filter(p => p.type === 'medicine')
        .reduce((sum, product) => sum + (product.returned || 0), 0);

      // 2. Tính tổng số lượng trả của vật tư y tế
      ...
      return {
        labels: ['Thuốc', 'Vật tư y tế'], 
        datasets: [
          {
            data: [totalMedicine, totalGoods], // Dữ liệu tổng đã trả
            backgroundColor: [
              '#42A5F5', 
              '#FFCA28'  
            ],
