<template>
    <div class="employees-page">
                    ...
                    <div class="search-wrapper">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" style="border-radius:8px;"
                                placeholder="Theo mã, tên nhân viên, điện thoại" v-model="searchQuery"
        ...
        <div class="table-container">
            <DataTable :value="filteredEmployees" v-model:expandedRows="expandedRows" stripedRows
                responsiveLayout="scroll" tableStyle="min-width: 50rem" :paginator="true" :rows="pagination.per_page"
                :totalRecords="pagination.total"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                :rowsPerPageOptions="[5, 10, 25]"
                currentPageReportTemplate="Hiển thị {first} đến {last} trong tổng số {totalRecords} nhân viên"
                dataKey="id" loadingIcon="pi pi-spinner" emptyMessage="Không có dữ liệu nhân viên">
                ...
                <template #expansion="slotProps">
                    <div class="employee-detail-container">
                        ...
                        <div class="detail-tabs">
                            ...
                            <button class="tab" :class="{ active: activeTab === 'salary' }"
                                        ...
                                        <table class="table table-sm table-borderless">
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span class="badge bg-info">{{ slotProps.data.department?.name
                                                            || '-' }}</span>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span class="badge bg-success">{{ slotProps.data.job_title?.name
                                                            || '-' }}</span>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span class="badge bg-primary">{{ slotProps.data.branch?.name ||
                                                            '-' }}</span>
                                                    </td>
                                                </tr>
                                                <tr>
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span class="badge bg-secondary">
                                                            {{ slotProps.data.salary_type === 'fixed' ? 'Lương cố định'
                                                            : 'Theo giờ' }}
                                                        </span>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    ...
                                                    <td class="text-success fw-bold">{{
                                                        formatCurrency(slotProps.data.salary_level) }}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                        ...
                                        <div v-if="slotProps.data.allowances && slotProps.data.allowances.length > 0">
                                            <div v-for="allowance in slotProps.data.allowances" :key="allowance.id"
                                                ...
                                                <div class="d-flex justify-content-between">
                                                    ...
                                                    <span class="text-success">{{ formatCurrency(allowance.amount)
                                                        }}</span>
                                                </div>
                                                <small class="text-muted">{{ getAllowanceTypeText(allowance.type)
                                                    }}</small>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div v-if="slotProps.data.targets && slotProps.data.targets.length > 0">
                                            <div v-for="target in slotProps.data.targets" :key="target.id"
                                                ...
                                                <small>Thưởng: {{ target.bonus_value }}{{ target.bonus_type ===
                                                    'percent' ? '%' : ' ₫' }}</small>
                                            </div>
                                        </div>
                                        ...
                                        <div v-if="slotProps.data.deductions && slotProps.data.deductions.length > 0">
                                            <div v-for="deduction in slotProps.data.deductions" :key="deduction.id"
                                                ...
                                                <div class="d-flex justify-content-between">
                                                    ...
                                                    <span class="text-danger">-{{ formatCurrency(deduction.amount)
                                                        }}</span>
                                                </div>
                                                <small class="text-muted">{{
                                                    getDeductionFrequencyText(deduction.frequency) }}</small>
                                            </div>
                                        </div>
                                        <div v-else class="text-muted">
        ...
        <CreateEmployeeModal :visible="showModal" @close="closeModal" @created="handleEmployeeCreated" />

        <!-- Edit Employee Modal -->
        <EditEmployeeModal :visible="showEditModal" :employeeId="selectedEmployeeId" @close="showEditModal = false"
            @updated="handleEmployeeUpdated" />



        <!-- Schedule Modal -->
        <ScheduleModal :visible="showScheduleModal" :employee="selectedEmployee" @close="showScheduleModal = false" />
    </div>
</template>
