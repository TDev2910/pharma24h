<template>
    <Dialog :visible="visible" @update:visible="$emit('close')" header="Cập nhật thông tin" :style="{ width: '1000px' }"
        :breakpoints="{ '960px': '75vw', '640px': '90vw' }" modal :closable="true" class="employee-modal">
                        ...
                        <div class="col-left">
                            ...
                            <div class="form-row">
                                <div class="form-field full-width">
                                    <label for="fullName" class="field-label">Họ và tên <span
                                            class="required">*</span></label>
                                    <InputText id="fullName" v-model="formData.full_name"
                                        placeholder="Ví dụ: Nguyễn Văn A" :class="{ 'p-invalid': errors.full_name }" />
                            ...
                            <div class="form-row">
                                <div class="form-field">
                                    ...
                                    <Calendar id="dob" v-model="formData.dob" dateFormat="dd/mm/yy"
                                        placeholder="dd/mm/yyyy" showIcon />
                                </div>
                                <div class="form-field">
                                    <label for="gender" class="field-label">Giới tính</label>
                                    <Dropdown id="gender" v-model="formData.gender" :options="genderOptions"
                                        optionLabel="label" optionValue="value" placeholder="Chọn giới tính" />
                                </div>
                            </div>
                            ...
                            <div class="form-row">
                                <div class="form-field">
                                    ...
                                    <InputText id="phone" v-model="formData.phone_number" placeholder="09xxxxxxxx"
                                        :class="{ 'p-invalid': errors.phone_number }" />
                                    <small v-if="errors.phone_number" class="p-error">{{ errors.phone_number[0]
                                        }}</small>
                                </div>
                                <div class="form-field">
                                    <label for="idCard" class="field-label">CMND/CCCD</label>
                                    <InputText id="idCard" v-model="formData.id_card_number"
                                        placeholder="Số giấy tờ tùy thân"
                                        :class="{ 'p-invalid': errors.id_card_number }" />
                                </div>
                            </div>
                            ...
                            <div class="form-row">
                                <div class="form-field full-width">
                                    ...
                                    <Textarea id="address" v-model="formData.address" rows="3"
                                        placeholder="Số nhà, đường, phường/xã..." autoResize />
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="info-group account-group">
                                ...
                                <div class="form-row flex-nowrap-row">
                                    <div class="form-field" style="flex: 1;">
                                        <label for="email" class="field-label">Email (Đăng nhập) <span
                                                class="required">*</span></label>
                                        <InputText id="email" v-model="formData.email" placeholder="email@company.com"
                                    ...
                                    <div class="form-field" style="flex: 1;">
                                        ...
                                        <div class="p-inputgroup custom-input-group">
                                            <Password id="password" v-model="formData.password" placeholder="Tự động..."
                                                :toggleMask="true" :feedback="false" class="w-full"
                                                inputClass="w-full border-noradius-right" />
                                            <Button icon="pi pi-key" @click="generatePassword"
                                                :loading="generatingPassword" severity="secondary"
                                                v-tooltip.top="'Tạo ngẫu nhiên'" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="info-group work-group mt-3">
                                ...
                                <div class="form-row">
                                    <div class="form-field">
                                        ...
                                        <div class="p-inputgroup custom-input-group">
                                            <InputText id="employeeCode" v-model="formData.employee_code"
                                                placeholder="Tự động" :class="{ 'p-invalid': errors.employee_code }" />
                                            <Button icon="pi pi-sync" @click="generateCode" :loading="generating"
                                                severity="secondary" v-tooltip.top="'Tạo mã tự động'" />
                                        </div>
                                        <small v-if="errors.employee_code" class="p-error">{{ errors.employee_code[0]
                                            }}</small>
                                    </div>

                                    <div class="form-field">
                                        <label for="startDate" class="field-label">Ngày vào làm</label>
                                        <Calendar id="startDate" v-model="formData.start_date" dateFormat="dd/mm/yy"
                                            placeholder="dd/mm/yyyy" showIcon />
                                    </div>
                                </div>
                                ...
                                <div class="form-row">
                                    <div class="form-field">
                                        ...
                                        <Dropdown id="branches" v-model="formData.branch_id"
                                            :options="resources.branches" optionLabel="name" optionValue="id"
                                            placeholder="Chọn chi nhánh" filter class="w-full" />
                                    </div>
                                    <div class="form-field">
                                        <label for="department" class="field-label">Phòng ban</label>
                                        <Dropdown id="department" v-model="formData.department_id"
                                            :options="resources.departments" optionLabel="name" optionValue="id"
                                            placeholder="Chọn phòng ban" filter class="w-full" />
                                    </div>
                                </div>
                                ...
                                <div class="form-row">
                                    <div class="form-field full-width">
                                        ...
                                        <Dropdown id="jobTitle" v-model="formData.job_title_id"
                                            :options="resources.job_titles" optionLabel="name" optionValue="id"
                                            placeholder="Chọn chức vụ" filter class="w-full" />
                                    </div>
                                </div>
                            </div>
                ...
                <TabPanel>
                    ...
                    <div class="salary-container">
                        <div class="salary-base-section">
                            <div class="form-row">
                                <div class="form-field" style="flex: 1;">
                                    <label for="salaryType" class="field-label">Hình thức trả lương <span
                                            class="required">*</span></label>
                                    <Dropdown id="salaryType" v-model="formData.salary_type"
                                        :options="salaryTypeOptions" optionLabel="label" optionValue="value"
                                        class="w-full" />
                                </div>
                                <div class="form-field" style="flex: 1;">
                                    <label for="salaryLevel" class="field-label">Mức lương cơ bản <span
                                            class="required">*</span></label>
                                    <InputNumber id="salaryLevel" v-model="formData.salary_level" mode="currency"
                                        currency="VND" locale="vi-VN" class="w-full font-bold" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="dynamic-sections">
                            <div class="dynamic-box" :class="{ 'active': formData.enable_allowance }">
                                <div class="box-header">
                                    <span class="box-title text-blue-600"><i class="pi pi-wallet mr-2"></i> Phụ
                                        cấp</span>
                                    <InputSwitch v-model="formData.enable_allowance" />
                                </div>
                                <div v-if="formData.enable_allowance" class="box-content">
                                    <div v-for="(allowance, index) in formData.allowances" :key="index"
                                        class="dynamic-row">
                                        <InputText v-model="allowance.name" placeholder="Tên phụ cấp" style="flex: 2" />
                                        <InputNumber v-model="allowance.amount" mode="currency" currency="VND"
                                            locale="vi-VN" placeholder="Số tiền" style="flex: 1.5" />
                                        <Dropdown v-model="allowance.type" :options="allowanceTypeOptions"
                                            optionLabel="label" optionValue="value" style="flex: 1.5" />
                                        <Button icon="pi pi-trash" class="p-button-danger p-button-text"
                                            @click="removeAllowance(index)" />
                                    </div>
                                    <Button label="Thêm dòng" icon="pi pi-plus" class="p-button-text p-button-sm"
                                        @click="addAllowance" />
                                </div>
                            </div>
                            ...
                            <div class="dynamic-box" :class="{ 'active': formData.enable_target_bonus }">
                                <div class="box-header">
                                    <span class="box-title text-green-600"><i class="pi pi-chart-line mr-2"></i> Thưởng
                                        chỉ tiêu /
                                        KPI</span>
                                ...
                                <div v-if="formData.enable_target_bonus" class="box-content">
                                    <div v-for="(target, index) in formData.targets" :key="index" class="dynamic-row">
                                        <InputText v-model="target.activity_type" placeholder="Hoạt động"
                                            style="flex: 2" />
                                        <InputNumber v-model="target.target_amount" mode="currency" currency="VND"
                                            locale="vi-VN" placeholder="Mức chỉ tiêu" style="flex: 1.5" />
                                        <Dropdown v-model="target.bonus_type" :options="bonusTypeOptions"
                                            optionLabel="label" optionValue="value" style="flex: 1" />
                                        <InputNumber v-model="target.bonus_value" placeholder="Giá trị thưởng"
                                            style="flex: 1.5" />
                                        <Button icon="pi pi-trash" class="p-button-danger p-button-text"
                                            @click="removeTarget(index)" />
                                    </div>
                                    <Button label="Thêm chỉ tiêu" icon="pi pi-plus" class="p-button-text p-button-sm"
                                        @click="addTarget" />
                                </div>
                            </div>
                            ...
                            <div class="dynamic-box" :class="{ 'active': formData.enable_deduction }">
                                <div class="box-header">
                                    <span class="box-title text-red-600"><i class="pi pi-minus-circle mr-2"></i> Các
                                        khoản giảm
                                        trừ</span>
                                    <InputSwitch v-model="formData.enable_deduction" />
                                </div>
                                <div v-if="formData.enable_deduction" class="box-content">
                                    <div v-for="(deduction, index) in formData.deductions" :key="index"
                                        class="dynamic-row">
                                        <InputText v-model="deduction.reason" placeholder="Lý do (VD: BHXH)"
                                            style="flex: 2" />
                                        <InputNumber v-model="deduction.amount" mode="currency" currency="VND"
                                            locale="vi-VN" placeholder="Số tiền" style="flex: 1.5" />
                                        <Dropdown v-model="deduction.frequency" :options="deductionFrequencyOptions"
                                            optionLabel="label" optionValue="value" style="flex: 1.5" />
                                        <Button icon="pi pi-trash" class="p-button-danger p-button-text"
                                            @click="removeDeduction(index)" />
                                    </div>
                                    <Button label="Thêm giảm trừ" icon="pi pi-plus" class="p-button-text p-button-sm"
                                        @click="addDeduction" />
                                </div>
                            </div>
                        </div>
        ...
        <template #footer>
            <div class="dialog-footer">
                <Button label="Hủy bỏ" icon="pi pi-times" class="p-button-text p-button-secondary"
                    @click="closeModal" />
                <Button label="Cập nhật thông tin" icon="pi pi-check" class="p-button-primary" @click="saveEmployee"
                    :loading="loading" />
            </div>
        </template>
    </Dialog>
    ...
    methods: {
        ...
        async loadEmployeeData() {
            ...
            try {
                ...
                const response = await axios.get(`/admin/employees/${this.employeeId}/edit`, {
                    headers: {
                        'X-CSRF-TOKEN': token,
                ...
                if (response.data && response.data.success) {
                    ...
                    // Map dữ liệu từ database vào form
                    this.formData = {
                        full_name: employee.full_name || '',
                        email: (employee.user && employee.user.email) ? employee.user.email : '',
                        ...
                        salary_level: employee.salary_level || 0,

                        // Load các mục bật/tắt
                        enable_allowance: employee.allowances && employee.allowances.length > 0,
                        enable_target_bonus: employee.targets && employee.targets.length > 0,
                        enable_deduction: employee.deductions && employee.deductions.length > 0,
                    ...
                    this.$toast.add({
                        severity: 'error',
                        summary: 'Lỗi',
                        detail: response.data?.message || 'Không thể tải thông tin nhân viên',
                        life: 3000
                    })
                }
            } catch (error) {
                ...
                this.$toast.add({
                    severity: 'error',
                    summary: 'Lỗi',
                    detail: `Không thể tải thông tin: ${error.response?.status || error.message}`,
                    life: 5000
                })
            }
        },
        ...
        async saveEmployee() {
            ...
            try {
                ...
                // SỬA: Dùng PUT thay vì POST
                const response = await axios.put(`/admin/employees/${this.employeeId}`, payload)

                if (response.data && response.data.success) {
                    this.$toast.add({
                        severity: 'success',
                        summary: 'Thành công',
                        detail: response.data.message || 'Đã cập nhật nhân viên thành công',
                        life: 3000
                    })

                    this.$emit('updated', response.data.employee || response.data)
                    this.closeModal()
                }
            } catch (error) {
...
@media (max-width: 768px) {
    ...
    /* Mobile cho phép xuống dòng lại */
}
</style>
