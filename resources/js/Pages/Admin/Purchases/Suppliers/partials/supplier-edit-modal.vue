<template>
  <Dialog v-model:visible="dialogVisible" header="Chỉnh sửa Nhà Cung Cấp" :style="{ width: '900px' }" :modal="true"
        ...
        <div class="row">
          ...
          <div class="col-md-6">
            ...
            <div class="mb-3">
              ...
              <Dropdown id="khu_vuc" v-model="formData.province" :options="provinceOptions" optionLabel="name"
                placeholder="-- Chọn tỉnh/thành --" class="w-100" :class="{ 'p-invalid': errors.khu_vuc }"
                :filter="true" :loading="loadingProvinces" />
              <small v-if="errors.khu_vuc" class="p-error">{{ errors.khu_vuc[0] }}</small>
            </div>
            ...
            <div class="mb-3">
              ...
              <Dropdown id="phuong_xa" v-model="formData.ward" :options="wardOptions" optionLabel="name"
                placeholder="-- Chọn tỉnh/thành trước --" class="w-100" :class="{ 'p-invalid': errors.phuong_xa }"
                :disabled="!formData.province || loadingWards" :filter="true" :loading="loadingWards" />
              <small v-if="errors.phuong_xa" class="p-error">{{ errors.phuong_xa[0] }}</small>
            </div>
...
export default {
  ...
  emits: ['update:visible', 'supplier-edited'],
  ...
  watch: {
    visible(newVal) {
      ...
      if (newVal) {
        // Load provinces trước, sau đó mới populate form
        this.loadProvinces().then(() => {
          // Populate form với dữ liệu từ supplier sau khi provinces đã load
          if (this.supplier) 
          {
            this.populateForm()
          }
        }).catch(() => {
          // Nếu load provinces thất bại, vẫn thử populate form
          if (this.supplier) {
            this.populateForm()
          }
        })
    ...
    // Watch supplier prop để populate form khi supplier thay đổi
    supplier: {
      immediate: true,
      handler(newSupplier) {
        if (newSupplier && this.visible && this.provinceOptions.length > 0) {
          this.populateForm()
        }
      }
    },

    //tự động load wards khi chọn province
  ...
  methods: {
    ...
    validateForm() {
      ...
      if (!this.formData.province) {
        this.errors.khu_vuc = ['Trường này là bắt buộc']
        isValid = false
      }

      if (!this.formData.ward) {
        this.errors.phuong_xa = ['Trường này là bắt buộc']
        isValid = false
      }
    ...
    async handleSubmit() {
      ...
      // Kiểm tra có supplier để biết là edit hay create
      if (!this.supplier || !this.supplier.id) {
        this.toast.add({
          severity: 'error',
          summary: 'Lỗi',
          detail: 'Không tìm thấy thông tin nhà cung cấp để cập nhật',
          life: 3000
        })
        return
      }
      ...
      try {
        // Sử dụng PUT để update
        router.put(`/admin/suppliers/${this.supplier.id}`, submitData, {
          preserveScroll: true,
          onSuccess: (page) => {
            this.toast.add({
              severity: 'success',
          ...
          onError: (errors) => {
            ...
            this.toast.add({
              severity: 'error',
              summary: 'Lỗi',
              detail: 'Có lỗi xảy ra khi cập nhật nhà cung cấp',
              life: 5000
            })
          },
          onFinish: () => {
    ...
    //tải dữ liệu từ supplier có trong db đã được thêm vào form
    populateForm() {
      if (!this.supplier) return

      // Populate form từ supplier
      this.form.ten_nha_cung_cap = this.supplier.ten_nha_cung_cap || ''
      this.form.ma_nha_cung_cap = this.supplier.ma_nha_cung_cap || ''
      this.form.dien_thoai = this.supplier.dien_thoai || ''
      this.form.email = this.supplier.email || ''
      this.form.nhom_nha_cung_cap_id = this.supplier.nhom_nha_cung_cap_id || null
      this.form.dia_chi = this.supplier.dia_chi || ''
      this.form.ten_cong_ty = this.supplier.ten_cong_ty || ''
      this.form.ma_so_thue = this.supplier.ma_so_thue || ''
      this.form.ghi_chu = this.supplier.ghi_chu || ''
      this.form.trang_thai = this.supplier.trang_thai === 'active'

      // Set province - vì khu_vuc trong database lưu tên, cần tìm object tương ứng
      if (this.supplier.khu_vuc && this.provinceOptions.length > 0) {
        const province = this.findProvinceByName(this.supplier.khu_vuc)
        if (province) {
          // Set province và chờ wards load
          this.formData.province = province

          // Nếu có ward trong supplier, đợi wards load xong rồi mới set
          if (this.supplier.phuong_xa) {
            // Đợi wards load xong (kiểm tra mỗi 100ms)
            const checkWards = () => {
              if (this.wardOptions.length > 0) {
                const ward = this.findWardByName(this.supplier.phuong_xa)
                if (ward) {
                  this.formData.ward = ward
                }
              } else if (!this.loadingWards) {
                // Nếu đã tải xong mà không có wards, có thể province không đúng
                // Retry sau 200ms
                setTimeout(checkWards, 200)
              } else {
                // Đang loading, tiếp tục đợi
                setTimeout(checkWards, 100)
              }
            }
            // Bắt đầu kiểm tra sau khi watcher đã trigger onProvinceChange
            this.$nextTick(() => {
              setTimeout(checkWards, 100)
            })
          }
    ...
    findProvinceByName(name) {
      if (!name || !this.provinceOptions.length) return null
      // Tìm exact match trước
      let province = this.provinceOptions.find(p => p.name === name)
      if (province) return province

      // Tìm partial match
      province = this.provinceOptions.find(p =>
    ...
    findWardByName(name) {
      if (!name || !this.wardOptions.length) return null
      // Tìm exact match trước (có thể có format "Ward Name (District Name)")
      let ward = this.wardOptions.find(w => w.name === name)
      if (ward) return ward

      // Tìm partial match - chỉ match phần đầu trước dấu ngoặc đơn
      ward = this.wardOptions.find(w => {
        const wardName = w.name.split(' (')[0] // Lấy phần trước " ("
        return wardName === name ||
          wardName.toLowerCase().includes(name.toLowerCase()) ||
          name.toLowerCase().includes(wardName.toLowerCase())
      })
      return ward || null
    },
