...
import '@Admin/orders/services.css';
...
export default {
  ...
  methods: {
    ...
    async loadBookings() {
      this.loading = true
      try {
        const response = await axios.get('/admin/service-bookings', {
          params: {
    ...
    async confirmBooking(booking) {
      if (confirm(`Xác nhận đặt lịch cho ${booking.customer_name}?`)) {
        try {
          const response = await axios.post(`/admin/service-bookings/${booking.id}/confirm`)
          if (response.data.success) {
            this.$toast.add({
              severity: 'success',
              summary: 'Thành công',
    ...
    async markAsPaid(booking) {
      if (confirm(`Đánh dấu đã thanh toán cho ${booking.customer_name}?`)) {
        try {
          const response = await axios.post(`/admin/service-bookings/${booking.id}/mark-paid`)
          if (response.data.success) {
            this.$toast.add({
              severity: 'success',
              summary: 'Thành công',
    ...
    async completeBooking(booking) {
      if (confirm(`Hoàn thành dịch vụ cho ${booking.customer_name}?`)) {
        try {
          const response = await axios.post(`/admin/service-bookings/${booking.id}/complete`)
          if (response.data.success) {
            this.$toast.add({
              severity: 'success',
              summary: 'Thành công',
    ...
    async cancelBooking(booking) {
      if (confirm(`Hủy đặt lịch cho ${booking.customer_name}?`)) {
        try {
          const response = await axios.post(`/admin/service-bookings/${booking.id}/cancel`)
          if (response.data.success) {
            this.$toast.add({
              severity: 'success',
              summary: 'Thành công',
