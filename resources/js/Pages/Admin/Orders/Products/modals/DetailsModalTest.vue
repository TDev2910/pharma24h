        ...
        <template #footer>
            <div class="dm-footer-wrapper">
                <div v-if="localOrder?.cancellation_status === 'requested'" class="dm-footer-note">
                    <Textarea v-model="adminNote" rows="2" style="width: 100%;height: 100%;"
                        placeholder="Ghi chú phản hồi cho khách (bắt buộc khi từ chối)..."
                        class="w-full dm-input-textarea" />
                </div>
                ...
                <div class="dm-footer-actions">
                    ...
                    <template v-if="localOrder?.cancellation_status === 'requested'">
                        ...
                        <Button label="Xác nhận hủy" icon="pi pi-check-circle" class="dm-btn-approve"
                    ...
                    <template v-if="localOrder?.delivery_method === 'shipping'">
                        <Button v-if="!localOrder.shipping_code && !localOrder.ghn_order_code" label="Tạo đơn GHN"
                            icon="pi pi-send" :loading="creatingGHN" @click="createGHNOrder"
                            :disabled="!localOrder.district_id && !localOrder.ward_code" class="dm-btn-ghn-create" />
                        <Button v-else-if="localOrder.ghn_order_code" label="Đồng bộ GHN" icon="pi pi-sync"
                            class="dm-btn-sync" :loading="syncingGHN" @click="syncGhnStatus" />
                        <Button v-else label="In Vận Đơn GHN" icon="pi pi-file-pdf" class="dm-btn-ghn-print" as="a"
                            :href="`/admin/orders-test/${localOrder.id}/ghn/print`" target="_blank" />
                    </template>
                </div>
            </div>
        </template>
