...
const printInvoice = () => {
  if (!localOrder.value?.id) return;
  window.open(`/admin/orders/${localOrder.value.id}/invoice`, '_blank');
};
...
const createGHNOrder = () => {
  ...
  router.post(`/admin/orders/${props.order.id}/ghn/create`, {}, {
...
const syncGhnStatus = async () => {
  ...
  router.post(`/admin/orders/${props.order.id}/ghn/sync`, {}, {
    preserveScroll: true,
    preserveState: true,
